...
class FCN(nn.Module):
  def __init__(self, input_dim: int, embedding_dim: int, n_hidden_layers: int,
               n_hidden_units: List[int], dropout_prob: float):
...
class Model:
  ...
  def __call__(self, text: str) -> np.ndarray:
    tokenized_text = self.tokenizer(text, return_tensors="pt")["input_ids"].to(
        self.device)
    with torch.no_grad():
